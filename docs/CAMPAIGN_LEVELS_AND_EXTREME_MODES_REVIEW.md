...
| Air/ground collision | Airborne helicopters incorrectly blocked ground units and the convoy through their map footprint. Ground movement, climbing and spawn checks now ignore airborne helicopters; landed aircraft still block traffic. Damaged-aircraft smoke follows its actual altitude. | Reproduced four blocked-aircraft cases; player, enemy, spider and convoy passage plus landed blocking regression. |
...
- The CI-discovered mobile compatibility-click fix passed the real skin-shop flow and a direct retargeted-click regression, including keyboard link activation after queued menu focus settles. The fixture waits for rendering frames instead of assuming a particular element retains native touch focus. Twenty other focused mobile/joystick/graphics checks also passed. The complete suite now runs across eight groups to keep the expanded release gate manageable.
- Current complete collection: **135 checks in 25 files**. The existing GitHub Pages workflow runs all checks across eight runners and only builds/deploys when every runner passes. Deployment status is authoritative in [GitHub Actions](https://github.com/buicongnguyen/Tankgame_3D/actions/workflows/deploy-pages.yml).
