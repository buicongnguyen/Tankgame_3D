...
Quick mouse clicks are retained until the next simulation frame, then consumed once; pause, blur and input reset cancel queued shots. CI distributes the full suite across eight independent runners, and the production build depends on all eight passing.
