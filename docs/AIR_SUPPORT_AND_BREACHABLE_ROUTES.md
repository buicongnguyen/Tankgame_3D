...
- Every mission receives supply calls: Easy and Crazy receive two; Normal and Hard receive one. Both options share a 28-second radio cooldown. Supply calls are separate from the limited 1-in-3 wreck drops; no unlimited healing or ammo loop.
- Supply payload: hull below 80% receives 40 HP (Easy / Normal / Hard) or 35 HP (Crazy); otherwise refill a depleted purchased special weapon with 2 arc rockets or 4 laser shots; otherwise deliver a 4-second shield. Existing health and ammo caps apply. A clear nearby landing site is required before spending the allowance.
