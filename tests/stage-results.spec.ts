...
test('last tank destruction gets 1.5 seconds before results and combat stops',async({page})=>{
 ...
 const r=await page.evaluate(()=>{const g=(window as any).__steel,hp=g.player.hp,time=g.elapsed,credits=g.save.credits;g.complete();g.step(1.49);return {phase:g.phase,stable:g.player.hp===hp&&g.elapsed===time&&g.save.credits===credits,active:g.input.active};});expect(r).toEqual({phase:'finishing',stable:true,active:false});await page.evaluate(()=>(window as any).__steel.step(.011));await expect(page.getByRole('heading',{name:'Mission accomplished'})).toBeVisible();await expect(page.getByText('Total earned')).toBeVisible();await page.screenshot({path:'test-results/stage-results-desktop.png'});
});
...
test('completed-stage replays pay credits without changing campaign progress',()=>{const save=freshSave();for(let level=0;level<3;level++)awardStage(save,0,30,100,100,level);const before={mission:save.mission,level:save.level,cleared:[...save.cleared],credits:save.credits};const r=awardStage(save,0,45,50,100,2);expect(r.replay).toBe(true);expect(r.base).toBe(90);expect(r.total).toBeGreaterThan(90);expect(save.credits).toBe(before.credits+r.total);expect(save.mission).toBe(before.mission);expect(save.level).toBe(before.level);expect(save.cleared).toEqual(before.cleared);});
