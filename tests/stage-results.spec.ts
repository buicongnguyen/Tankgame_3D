...
test('time and hull bonuses are bounded, saved and awarded only once',()=>{
 const save=freshSave(),r=awardStage(save,0,45,50,100);expect(r).toMatchObject({base:180,timeBonus:23,healthBonus:23,total:226,healthPercent:50});expect(parseSave(JSON.stringify(save)).credits).toBe(226);expect(awardStage(save,0,0,100,100)).toMatchObject({replay:true,total:0});expect(save.credits).toBe(226);expect(awardStage(freshSave(),0,200,200,100)).toMatchObject({timeBonus:0,healthBonus:45});expect(awardStage(freshSave(),3,45,100,100)).toMatchObject({timeBonus:0,healthBonus:70,target:0});
});
