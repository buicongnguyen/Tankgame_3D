...
test('Q and collected shields protect both vehicles from bullets and explosions, expire, pause, and reset',async({page})=>{
 await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();await page.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};g.start(2,0),g.clearOpening();g.world.covers=[];g.world.activities=[];g.enemies=[];g.world.navigationRevision++;g.convoy.position.set(0,0,0);g.player.visual.root.position.set(30,0,30);});
 await page.keyboard.press('q');const r=await page.evaluate(async()=>{const g=(window as any).__steel;g.updatePlayer(.01);const field=g.convoy.getObjectByName('ConvoyShield'),visible=field.visible,initial=g.convoyHealth,hp=g.player.hp;
  ...
  const {createActivity}=await import('/src/three/activities.ts');const a=createActivity(g.world.entities,'shield',30,30,4);g.world.activities=[a];g.updateActivities(.01);g.updatePlayer(.01);const now=g.convoyHealth;g.damageConvoy(100);const pickup=a.spent&&field.visible&&g.convoyHealth===now;g.updateHud();const hud=g.el('objective').textContent.includes('SHIELDED')&&!g.el('objective').textContent.includes('400%');g.start(2,0),g.clearOpening();return {visible,protectedBoth,paused,expired,bullet,blast,pickup,hud,reset:g.convoyHealth===1040&&g.shieldTime===0&&!g.convoy.getObjectByName('ConvoyShield').visible};
