...
test('clearing all hostiles completes from anywhere, waits for reserves, and allows a surviving escort to finish early',async({page})=>{
 await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();const r=await page.evaluate(()=>{
  const g=(window as any).__steel;g.frame=()=>{};g.start(0);const reserved=g.enemies.at(-1);for(const e of g.enemies)if(e!==reserved)e.dead=true;g.player.visual.root.position.set(g.world.layout.spawn.x,0,g.world.layout.spawn.z);g.step(.01);const waits=g.phase==='playing';reserved.dead=true;const credits=g.save.credits;g.step(.01);const early=g.phase==='finishing'&&g.el('objective').textContent==='AREA SECURED';const award=g.save.credits;g.step(1.49);const delay=g.phase==='finishing';g.step(.02);const results=g.phase==='depot';g.complete();const once=g.save.credits===award&&award>=credits;
