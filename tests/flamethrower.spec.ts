...
for(const viewport of [{width:1440,height:900},{width:390,height:844},{width:844,height:390}])test(`flame shop, ninth weapon, Easy hull and visible fire fit ${viewport.width}`,async({browser})=>{
 ...
 await page.reload();await page.getByRole('button',{name:'DEPLOY'}).click();await expect(page.locator('#weapon-label')).toContainText('Flamethrower');await expect(page.locator('#health-label')).toHaveText('1440 / 1440');await ctx.close();
});
