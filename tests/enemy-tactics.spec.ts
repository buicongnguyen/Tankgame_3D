...
for(const mobile of [false,true])test(`guard landmarks stay readable and preserve patrol state in Low detail on ${mobile?'mobile':'desktop'}`,async({browser})=>{
 const context=await browser.newContext({viewport:mobile?{width:390,height:844}:{width:1280,height:800},hasTouch:mobile,isMobile:mobile}),page=await context.newPage(),errors:string[]=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();await page.evaluate(()=>{const g=(window as any).__steel;g.save.difficulty='normal';g.start(0,0);});
 const high=await page.evaluate(async mobile=>{const g=(window as any).__steel;g.frame=()=>{};await g.world.load(false);g.world.settings(false);g.start(2,0);const squad=g.enemies.filter((e:any)=>e.encounter.group===1),anchor=squad.reduce((p:any,e:any)=>({x:p.x+e.visual.root.position.x/squad.length,z:p.z+e.visual.root.position.z/squad.length}),{x:0,z:0});g.world.scene.fog=null;g.world.camera.position.set(anchor.x+10,mobile?45:35,anchor.z+30);g.world.camera.lookAt(anchor.x,0,anchor.z);g.updateHud();g.world.renderer.render(g.world.scene,g.world.camera);return {triangles:g.world.renderer.info.render.triangles};},mobile);await page.screenshot({path:`test-results/guard-posts-${mobile?'mobile':'desktop'}-detailed.png`});
