...
test('track contact requires actual speed, respects cover and counts only infantry',async({page})=>{
 ...
 const result=await page.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};
  ...
  let e=setup();g.moveUnit(g.player,7,0,.7);const fast=e.dead&&g.infantryKills===1&&g.kills===0&&g.world.activities.length===7;g.moveUnit(g.player,-7,0,.7);const once=g.infantryKills===1;
