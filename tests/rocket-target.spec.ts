...
test('touch aiming previews and hits the nearer target over cover; launch destination stays fixed',async({page})=>{
 ...
 expect(result.preview).toBe(6);expect(result.target).toBe(6);expect(result.radius).toBeCloseTo(8);expect(result.airborne).toBeGreaterThan(15);expect(result.hit).toBe(true);expect(result.reset).toBe(1);
});
