...
test('travel stages increase route length with each level and O roads close without overlap',()=>{
 for(let stage=0;stage<MISSIONS.length;stage++){if(['capture','defense'].includes(MISSIONS[stage].kind))continue;const layouts=[0,1,2].map(level=>stageLayout(stage,level,levelMission(stage,level).kind));expect(layouts[1].length,`stage ${stage}`).toBeGreaterThan(layouts[0].length);expect(layouts[2].length,`stage ${stage}`).toBeGreaterThan(layouts[1].length);
...
for(const reverse of [false,true])test(`O convoy waits for a free choice and completes ${reverse?'left':'right'} circuit`,async({page})=>{
 await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();const r=await page.evaluate(async reverse=>{const g=(window as any).__steel;g.frame=()=>{};g.start(2,1);for(const e of g.enemies)e.dead=true;const start=g.world.layout.points[0];g.updateConvoy(1);g.updateHud();const waited=g.convoyDistance===0&&g.convoyReverse===null&&g.el('objective').textContent.includes('CHOOSE LEFT OR RIGHT');g.player.visual.root.position.set(start.x+(reverse?-5:5),0,start.z);g.updateConvoy(0);const selected=g.convoyReverse===reverse;const {alongRoute}=await import('/src/three/stage-layout.ts');const {circleBox}=await import('/src/three/rules.ts');let n=0,clipped=false;
