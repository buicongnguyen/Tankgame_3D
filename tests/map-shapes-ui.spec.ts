...
for(const mobile of [false,true])test(`shaped route presentation ${mobile?'mobile':'desktop'}`,async({browser})=>{
 ...
 for(const [stage,level,label] of [[4,0,'S'],[4,1,'flipped-S'],[5,0,'L'],[4,2,'U']] as const){
  const result=await page.evaluate(async({stage,level,mobile})=>{const g=(window as any).__steel;g.frame=()=>{};g.start(stage,level);const {alongRoute}=await import('/src/three/stage-layout.ts');const p=alongRoute(g.world.layout.points,g.world.layout.length*.35);g.player.visual.root.position.set(p.x,0,p.z);g.syncVisual(g.player);g.world.target.copy(g.player.visual.root.position);const offset=g.world.cameraOffset();g.world.target.x+=offset.x;g.world.target.z+=offset.z;g.world.update(0,g.player.visual.root.position);if(!mobile){g.world.scene.fog=null;g.world.camera.position.set(0,145,110);g.world.camera.lookAt(0,0,0);}g.world.renderer.render(g.world.scene,g.world.camera);g.updateHud();return {shape:g.world.layout.shape,overflow:document.documentElement.scrollWidth>innerWidth,calls:g.world.renderer.info.render.calls};},{stage,level,mobile});
