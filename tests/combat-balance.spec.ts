...
test('exact balance and background allowance migrate and bank without replay farming',()=>{
 expect([WEAPONS[8].price,WEAPONS[8].capacity]).toEqual([800,80]);expect(WEAPONS[8].damage).toBeCloseTo(24*.8);expect(FLAME.burnDps).toBeCloseTo(18*.8);
 expect([BARRAGE.count,BARRAGE.damage]).toEqual([6,90]);
 expect(ammoCaps('classic')).toEqual([12,6,3,80]);expect(ammoCaps('quartermaster')).toEqual([15,8,4,200]);expect(ammoCapacity('quartermaster',6)).toBe(8);
