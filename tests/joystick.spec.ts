...
test('aim trigger hysteresis and nub containment',async({browser})=>{
 const context=await browser.newContext({viewport:{width:320,height:568},isMobile:true,hasTouch:true});const page=await context.newPage();await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).tap();const cdp=await context.newCDPSession(page);
