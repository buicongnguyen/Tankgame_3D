...
test('aircraft clear ground traffic while landed helicopters block it',async({page})=>{
 await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();const result=await page.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};g.start(15,2);g.world.covers=[];g.convoy=null;const heli=g.enemies.find((u:any)=>u.bossKind==='helicopter');g.enemies=[heli];heli.visual.root.position.set(0,6.5,0);g.player.visual.root.position.set(-6,0,0);g.moveUnit(g.player,12,0,1);const player=g.player.visual.root.position.x>0;g.player.visual.root.position.set(20,0,20);const tank=g.makeUnit(-6,0,'raider');g.enemies.push(tank);g.moveUnit(tank,12,0,1);const enemy=tank.visual.root.position.x>0;tank.dead=true;const spider=g.makeUnit(-10,0,'boss','spider');g.enemies.push(spider);g.bosses.climb(g,spider,12,0,1);const climbing=spider.visual.root.position.x>0;spider.dead=true;heli.visual.root.position.y=0;g.player.visual.root.position.set(-6,0,0);g.moveUnit(g.player,12,0,1);const landed=g.player.visual.root.position.x<0;
 g.start(15,2);g.world.covers=[];const b=g.enemies.find((u:any)=>u.bossKind==='helicopter');g.enemies=[b];g.updateEnemies=()=>{};b.visual.root.position.set(0,6.5,-49);g.step(.1);const convoyFlying=g.convoy.position.z>-50;g.convoy.position.z=-50;g.convoyDistance=0;b.visual.root.position.y=0;g.step(.1);return {player,enemy,climbing,landed,convoyFlying,convoyLanded:g.convoyBlocked&&g.convoy.position.z===-50};});expect(result).toEqual({player:true,enemy:true,climbing:true,landed:true,convoyFlying:true,convoyLanded:true});
});
