...
test('aircraft clear ground traffic while landed helicopters block it',async({page})=>{
 ...
 g.start(15,2);g.world.covers=[];const b=g.enemies.find((u:any)=>u.bossKind==='helicopter');g.enemies=[b];g.updateEnemies=()=>{};b.visual.root.position.set(0,6.5,47);g.step(.1);const convoyFlying=g.convoy.position.z<48;g.convoy.position.z=48;b.visual.root.position.y=0;g.step(.1);return {player,enemy,climbing,landed,convoyFlying,convoyLanded:g.convoyBlocked&&g.convoy.position.z===48};});expect(result).toEqual({player:true,enemy:true,climbing:true,landed:true,convoyFlying:true,convoyLanded:true});
});
