...
test('environment cover, salvage, water traction, spawn clearance and new objectives',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{const g=(window as any).__steel;g.save.difficulty='normal';const {circleBox}=await import('/src/three/rules.ts');const blocked=[];
 ...
 const stone=g.world.covers.find((c:any)=>c.kind==='stonewall'),steel=g.world.covers.find((c:any)=>c.kind==='steelwall'),house=g.world.covers.find((c:any)=>c.kind==='house');g.hitCover(stone,999);g.hitCover(steel,999);g.hitCover(house,999);const salvage=g.world.activities.length;g.hitCover(house,999);const cover=stone.hp<=0&&!stone.mesh.visible&&steel.hp===Infinity&&salvage===7&&g.world.activities.length===7;
 for(const e of g.enemies){e.dead=false;g.damageUnit(e,9999,g.player.visual.root.position);}g.player.visual.root.position.copy(g.world.ring.position);g.step(.01);g.step(g.finishDelay);const river=g.phase;g.start(7);for(const e of g.enemies)e.dead=true;const end=g.world.layout.points.at(-1);g.convoyDistance=g.world.layout.length-.01;g.convoy.position.set(end.x,0,end.z-.01*Math.sign(end.z));g.player.visual.root.position.set(end.x+4,0,end.z);g.step(.1);g.step(g.finishDelay);const snow=g.phase;g.start(8,2);for(const e of g.enemies)e.dead=true;g.elapsed=44.99;g.step(.02);g.step(g.finishDelay);return {blocked,wet,bridge,cover,river,snow,ridge:g.phase};});
