...
test('environment cover, salvage, water traction, spawn clearance and new objectives',async({page})=>{
 ...
 for(let i=0;i<9;i++){g.start(i);for(const u of [g.player,...g.enemies])if(g.world.covers.some((c:any)=>c.hp>0&&circleBox(u.visual.root.position,u.role==='boss'?2:1.25,c)))blocked.push([i,u.role,u.visual.root.position.x,u.visual.root.position.z]);}
