...
for(const difficulty of DIFFICULTIES)test(`${difficulty} layouts keep road weapons contested and recovery off-road`,()=>{
 for(let stage=0;stage<16;stage++)for(let level=0;level<3;level++){
  ...
  expect(useful.filter(s=>s.kind==='repair')).toHaveLength(1);expect(useful.filter(s=>s.kind!=='repair')).toHaveLength(difficulty==='easy'?1:0);
...
test('campaign includes eastward, northward, diagonal and southward journeys',()=>{
 for(const [stage,direction] of [[4,'east'],[13,'east'],[0,'north'],[5,'northeast'],[10,'northeast'],[11,'northeast'],[7,'south']] as const){const layout=stageLayout(stage);expect(layout.direction).toBe(direction);expect(layout.points.length).toBeGreaterThanOrEqual(2);const end=alongRoute(layout.points,layout.length);expect(end).toEqual(layout.points.at(-1));}
});

test('waiting route groups patrol locally, wake near their zone and counterattack when hit',async({page})=>{
 await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();const r=await page.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};g.start(4,2);const before=g.enemies.map((e:any)=>e.visual.root.position.clone());
  for(let i=0;i<300;i++){g.elapsed+=.05;g.updateEnemies(.05);}const quiet=g.enemies.every((e:any,i:number)=>(e.patrol?e.visual.root.position.distanceTo(before[i])<=e.patrol.radius+2:e.visual.root.position.equals(before[i]))&&!e.encounter.active),noShots=g.shots.length===0;
...
test('salvage is limited by difficulty and stays near reachable wreck positions',async({page})=>{
 await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();const rows=await page.evaluate(async()=>{const g=(window as any).__steel;g.frame=()=>{};const {roadDistance,alongRoute}=await import('/src/three/stage-layout.ts');const {circleBox}=await import('/src/three/rules.ts');return ['easy','normal','hard','crazy'].map(difficulty=>{g.save.difficulty=difficulty;g.start(4);g.lootRandom=()=>.1;const sources:any[]=[];const before=g.world.activities.length;for(let kills=1;kills<=128;kills++){g.kills=kills;const source=alongRoute(g.world.layout.points,g.world.layout.length*((kills%7+1)/8));sources.push(source);g.dropSalvage(source);}const drops=g.world.activities.slice(before);return {difficulty,count:drops.length,nearWreck:drops.every((p:any)=>sources.some(s=>Math.hypot(p.x-s.x,p.z-s.z)<=8.01)),clear:drops.every((p:any)=>!g.world.covers.some((c:any)=>c.hp>0&&circleBox(p,2.85,c)))};});});expect(rows.map((r:any)=>r.count)).toEqual([6,4,4,5]);expect(rows.every((r:any)=>r.nearWreck&&r.clear)).toBe(true);
});
