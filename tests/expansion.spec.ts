...
test('each level checkpoints and the finale waits for all four bosses',async({page})=>{
 ...
 const result=await page.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};const checkpoints=[];
  for(let level=0;level<2;level++){g.start(0,level);for(const e of g.enemies)g.damageUnit(e,999999,g.player.visual.root.position,true);g.step(.02);g.step(.8);checkpoints.push([g.save.mission,g.save.level,g.save.cleared[0],g.phase]);}
  g.save.difficulty='crazy';g.start(0,2);const bosses=g.enemies.filter((e:any)=>e.role==='boss');for(const e of g.enemies)if(!bosses.includes(e))g.damageUnit(e,999999,g.player.visual.root.position,true);for(const e of bosses.slice(0,3))g.damageUnit(e,999999,g.player.visual.root.position,true);g.step(.02);const waits=g.phase==='playing';g.damageUnit(bosses[3],999999,g.player.visual.root.position,true);g.step(.02);const delay=g.phase==='finishing';g.step(.79);const still=g.phase==='finishing';g.step(.02);checkpoints.push([g.save.mission,g.save.level,g.save.cleared[0],g.phase]);return {checkpoints,waits,delay,still};
