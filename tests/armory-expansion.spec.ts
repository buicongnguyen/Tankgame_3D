...
test('all systems and owned weapons reach level 20 with prices, limits and old-save migration',()=>{
 ...
 const s=parseSave(JSON.stringify(old));expect(s.weaponLevels).toEqual(Array(8).fill(0));expect(s.upgrades).toEqual({armor:0,power:2,reload:3,engine:0,shield:0});expect(s.equippedWeapon).toBe(4);expect(s.credits).toBe(100000);
 expect(upgradeWeapon(s,6)).toBe(false);for(const id of [1,2,5,6,7])expect(buyWeapon(s,id)).toBe(true);
 ...
 for(let id=0;id<WEAPONS.length;id++){s.credits=100000;for(let n=0;n<20;n++){expect(upgradeWeapon(s,id)).toBe(true);expect(s.weaponLevels[id]).toBe(n+1);}const before=s.credits;expect(upgradeWeapon(s,id)).toBe(false);expect(s.credits).toBe(before);expect(reloadSeconds(s,id)).toBeGreaterThanOrEqual(id===1?.09:id===5?.14:.18);}
...
test('triple arc has three real airborne rockets per volley, three volleys total, and no normal ammo refill',async({page})=>{
 ...
 for(let i=0;i<2;i++){g.weapon=7;g.shoot(g.player,true);}const count=g.special.arcs.length,empty=g.specialAmmo[2]===0&&g.weapon===0&&!g.weaponAvailable(7);g.weapon=7;g.shoot(g.player,true);const noExtra=g.special.arcs.length===count;
...
for(const viewport of [{width:1440,height:900},{width:320,height:568},{width:390,height:844},{width:844,height:390}])test(`level-20 shop and eight-weapon controls fit and persist ${viewport.width}`,async({browser})=>{
 ...
 await p.reload();await press('[data-action=deploy]');await p.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};});await expect(p.locator('#weapon-label')).toContainText('Triple arc launcher · 3 volleys');await press('#weapon');await expect(p.locator('#weapon-picker button')).toHaveCount(8);
