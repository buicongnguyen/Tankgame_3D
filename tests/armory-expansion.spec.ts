...
test('missile impact splashes nearby units, building edges and local concrete while preserving hard terrain',async({page})=>{
 ...
 const cover=(kind:string,x:number,z:number,w:number,d:number,hp:number)=>{const mesh=g.world.clone(kind);mesh.position.set(x,0,z);g.world.arena.add(mesh);const c={kind,x,z,w,d,hp,mesh};g.world.covers.push(c);return c;};const house=cover('house',10,10,10,4,1000),rock=cover('volcanic-rock',3,13,2,2,Infinity);
...
for(const viewport of [{width:1440,height:900},{width:320,height:568},{width:390,height:844},{width:844,height:390}])test(`level-20 shop and eight-weapon controls fit and persist ${viewport.width}`,async({browser})=>{
 ...
 for(const id of [5,6,7])await press(`[data-action=buy-weapon][data-value="${id}"]`);await press('[data-action=upgrade-weapon][data-value="5"]');await press('[data-action=shop-tab][data-value=systems]');await press('[data-action=buy][data-value=engine]');await press('[data-action=shop-tab][data-value=equipment]');
 ...
 await p.evaluate(()=>(window as any).__steel.storage.flush());await p.reload();await press('[data-action=deploy]');await p.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};});await expect(p.locator('#weapon-label')).toContainText('Triple arc launcher · 3 volleys');await press('#weapon');await expect(p.locator('#weapon-picker button')).toHaveCount(9);
