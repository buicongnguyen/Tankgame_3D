...
test('lethal damage cannot be repaired and approaching enemies retain fire warning',async({page})=>{
 ...
 const result=await page.evaluate(()=>{const g=(window as any).__steel;const pad=g.world.activities.find((a:any)=>a.kind==='repair');g.player.visual.root.position.set(pad.x,0,pad.z);g.damageUnit(g.player,9999,pad);g.step(1/60);const failed=g.phase==='failed';g.start(0);g.player.visual.root.position.set(0,0,0);for(const e of g.enemies)e.cooldown=10;const e=g.enemies[1];e.visual.root.position.set(0,0,-60);e.cooldown=-4;g.updateEnemies(.1);e.visual.root.position.set(0,0,-20);g.updateEnemies(.1);return {failed,warning:e.cooldown>0&&e.visual.beam.visible,shots:g.shots.length};});expect(result).toEqual({failed:true,warning:true,shots:0});
});
...
test('WebKit mobile startup and touch action controls',async()=>{
 const browser=await webkit.launch();const page=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true});const errors:string[]=[];page.on('pageerror',e=>errors.push(e.message));
