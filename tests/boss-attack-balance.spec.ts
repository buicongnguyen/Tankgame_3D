...
test('light laser pulses accelerate consistently at 30 and 60 fps with bounded effects',async({page})=>{
 await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();
 const rows=await page.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};return [30,60].map(fps=>{g.start(0),g.clearOpening();g.world.covers=[];const b=g.makeUnit(0,0,'boss','laser');g.enemies=[b];b.visual.root.position.set(0,0,0);g.player.visual.root.position.set(0,0,30);g.player.hp=100000;g.bosses.update(g,b,4.1);const hp=g.player.hp;g.bosses.update(g,b,1.51);const warned=hp===g.player.hp;
...
for(const kind of ['missile','helicopter'])test(`${kind} warns its wider blast area and leaves space to dodge`,async({page})=>{
 await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();
 const rows=await page.evaluate(kind=>{const g=(window as any).__steel;g.frame=()=>{};return [-14,-14.6].map(x=>{g.start(0),g.clearOpening();g.world.covers=[];const b=g.makeUnit(0,0,'boss',kind);g.enemies=[b];b.visual.root.position.set(0,0,0);g.player.visual.root.position.set(0,0,30);g.bosses.update(g,b,4.1);const s=g.bosses.states.get(b),targets=JSON.stringify(s.targets),radii=s.markers.map((m:any)=>m.geometry.parameters.outerRadius),warning=s.time,markers=[...s.markers],rockets=[...s.rockets];
...
test('heavy rail warning matches its wider shot, with cover and sidestep protection',async({page})=>{
 await page.goto('/?e2e');await page.getByRole('button',{name:'DEPLOY'}).click();const r=await page.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};g.start(0),g.clearOpening();const b=g.makeUnit(0,0,'boss','rail');g.enemies=[b];g.world.covers=[];b.visual.root.position.set(0,0,0);g.player.visual.root.position.set(0,0,30);g.bosses.update(g,b,4.1);g.bosses.update(g,b,.1);const warningWidth=b.visual.beam.geometry.parameters.width*b.visual.beam.scale.x;
