...
test('all non-boss enemies double and grow two percent per campaign level',()=>{
 const roles=['rifleman','rocketeer','jeep','raider','sentry','heavy'] as const;
 expect(roles.map(r=>enemyHealth(r,0,0))).toEqual([70,110,150,220,300,480]);
 expect(roles.map(r=>enemyHealth(r,0,1))).toEqual([71,112,153,224,306,490]);
 expect(roles.map(r=>enemyHealth(r,15,2))).toEqual([136,213,291,427,582,931]);
 for(const role of roles){let previous=0;for(let stage=0;stage<MISSIONS.length;stage++)for(let level=0;level<3;level++){const hp=enemyHealth(role,stage,level);expect(hp).toBeGreaterThan(previous);previous=hp;}}
 expect(enemyHealth('heavy',100,2)).toBe(960);expect(enemyHealth('raider',-2,-1)).toBe(220);
});
...
test('real cannon preserves infantry kills and rewards flanking tougher tanks',async({page})=>{
 await arena(page);const rows=await page.evaluate(()=>{
  ...
  for(const role of ['rifleman','rocketeer','jeep','raider','sentry','heavy'])for(const [facing,x,z] of [['front',0,20],['side',20,0],['rear',0,-20]] as [string,number,number][]){
   ...
   while(!e.dead&&hits<60){g.shoot(g.player,true);for(let i=0;i<50;i++)g.updateShots(1/60);hits++;g.syncVisual(e);if(hits===1)firstBar=e.visual.bar.scale.x;}
 ...
 const expected:Record<string,number[]>={rifleman:[2,2,2],rocketeer:[3,3,3],jeep:[4,4,4],raider:[8,5,4],sentry:[11,7,5],heavy:[17,11,8]};
...
test('missile direct plus splash and laser remain effective; upgrades beat late armor',async({page},testInfo)=>{
 await arena(page);const rows=await page.evaluate(()=>{
  ...
  for(const stage of [0,15])for(const upgraded of [false,true])for(const weapon of [0,2,3,4,6]){
   ...
   while(!e.dead&&hits<60){g.shoot(g.player,true);for(let i=0;i<100;i++){g.updateShots(1/60);g.special.update(g,1/60);}hits++;}
 ...
 for(const row of rows){expect(row.dead,JSON.stringify(row)).toBe(true);expect(row.max).toBe(row.stage===0?480:931);}
 ...
 for(const w of [0,2,3,4,6]){expect(hits(15,true,w)).toBeLessThan(hits(15,false,w));}
});
...
test('all ten bosses retain individual HP and meaningful exposed cores',async({page})=>{
 ...
 expect(rows.map(r=>r.max)).toEqual([960,1020,1260,816,1104,936,1200,1320,1440,1080]);for(const r of rows){expect(r.protectedDamage).toBeCloseTo(18.59);expect(r.exposedDamage).toBeCloseTo(50.05);expect(r.alive).toBe(true);}
});
...
test('a tougher final tank reaches delayed results and awards the stage once',async({page})=>{
 await arena(page);const r=await page.evaluate(()=>{
  ...
  for(let shot=0;shot<7;shot++){g.shoot(g.player,true);for(let i=0;i<50;i++)g.updateShots(1/60);}g.step(.001);const survived=g.phase==='playing'&&!e.dead;
