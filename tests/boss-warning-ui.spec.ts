...
for(const mobile of [false,true])test(`boss warnings stay accurate in ${mobile?'mobile':'desktop'} Detailed and Low`,async({browser})=>{
 ...
 await page.evaluate(()=>{const g=(window as any).__steel;g.frame=()=>{};g.start(6,2);const b=g.enemies.find((u:any)=>u.role==='boss');for(const e of g.enemies)if(e!==b){e.dead=true;e.visual.root.visible=false;}g.enemies=[b];b.encounter.active=true;b.visual.root.position.set(0,0,0);g.player.visual.root.position.set(0,0,26);g.bosses.update(g,b,4.1);g.updateHud();});
