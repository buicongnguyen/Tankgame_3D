...
const names=['tank','transport','barricade','crate','barrel','relay','pine','house','stonewall','steelwall','bridge','hill','rifleman','rocketeer'];
let total=0;
for(const name of names){
  ...
  if(['tank','rifleman','rocketeer'].includes(name))for(const node of ['Hull','Turret','Muzzle'])assert.ok(json.nodes.some(n=>n.name===node),`Missing ${node}`);
