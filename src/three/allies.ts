...
import {clearSight,seesTarget} from './encounters';
...
export class Allies {
 tanks:RescueTank[]=[];trail:Point[]=[];gun:T.Group|null=null;cooldown=2;burst=0;scanAt=0;
 clear(){for(const a of this.tanks){a.marker.removeFromParent();a.marker.geometry.dispose();(a.marker.material as T.Material).dispose();}this.tanks=[];this.trail=[];this.gun=null;this.cooldown=2;this.burst=0;this.scanAt=0;}
 ...
 update(g:Game,dt:number){
  ...
  for(const [i,a] of this.tanks.entries()){
   ...
   if(g.elapsed>=(a.scanAt??0)){a.scanAt=g.elapsed+.2;a.target=g.enemies.filter(e=>!e.dead&&!e.pending&&e.encounter?.active!==false&&!g.airborne(e)&&distance(p,e.visual.root.position)<26&&clearSight(g,p,e.visual.root.position)).sort((a,b)=>distance(p,a.visual.root.position)-distance(p,b.visual.root.position))[0];}const target=a.target&&!a.target.dead&&!a.target.pending&&a.target.encounter?.active!==false&&distance(p,a.target.visual.root.position)<26&&clearSight(g,p,a.target.visual.root.position)?a.target:undefined;
   ...
   if((gap>5+i*2&&distance(p,destination)>2)||blockedConvoy){if(!a.next||g.elapsed>=(a.repathAt??0)||distance(p,a.next)<.7){a.next=g.navigation.next(g.world,p,destination)??undefined;a.repathAt=g.elapsed+.35;}let next=a.next;if(!g.world.covers.some(c=>c.hp>0&&segmentBox(p,destination,c,1.4)!==null))next=destination;
  ...
  if(this.gun&&g.convoy&&g.convoyHealth>0){this.cooldown=Math.max(0,this.cooldown-dt);const p=g.convoy.position,missile=g.level===0;
   const targets=g.enemies.filter(e=>!e.dead&&!e.pending&&e.encounter?.active!==false&&!g.airborne(e)&&distance(p,e.visual.root.position)<(missile?24:22)&&seesTarget(g,e,p));targets.sort((a,b)=>(missile?Number(g.isInfantry(a))-Number(g.isInfantry(b)):Number(!g.isInfantry(a))-Number(!g.isInfantry(b)))||distance(p,a.visual.root.position)-distance(p,b.visual.root.position));
   const target=targets[0];if(target){const angle=Math.atan2(target.visual.root.position.x-p.x,target.visual.root.position.z-p.z),turret=this.gun.getObjectByName('Turret')!;turret.rotation.y=angle-g.convoy.rotation.y;
 ...
 progress(g:Game){if(this.saved<2)return this.saved/3;return this.survivors>0&&(g.battlefieldClear()||distance(g.player.visual.root.position,g.world.layout.points.at(-1)!)<7)?1:.9;}
}
