...
export interface RecordEntry {pilot:string;mission:number;level:number;difficulty:string;score:number;seconds:number;health:number;date:string}
...
export function bestRecords(rows:RecordEntry[],entry:RecordEntry){
 const same=(r:RecordEntry)=>r.pilot===entry.pilot&&r.mission===entry.mission&&r.level===entry.level&&r.difficulty===entry.difficulty;
...
export function recordRun(mission:number,level:number,difficulty:string,seconds:number,health:number,target:number){
 memory=bestRecords(read(),{pilot:pilotName(),mission,level,difficulty,seconds,health,score:scoreRun(seconds,health,target),date:new Date().toISOString()});
...
export function showLeaderboard(root:HTMLElement,mission:number,level:number,difficulty:string,back:()=>void){
 root.innerHTML='<section class="panel" style="width:min(720px,94vw);max-height:85dvh;overflow:auto"><span class="eyebrow">LOCAL LEADERBOARD</span><h1>Best pilots</h1><p>Saved on this device. Campaign loadouts apply.</p><label>Pilot name <input id="rank-name" maxlength="20" aria-label="Pilot name"></label><button id="rank-save">Save name</button><p id="rank-message" role="status"></p><div style="display:flex;gap:8px;flex-wrap:wrap"><select id="rank-stage" aria-label="Stage"></select><select id="rank-level" aria-label="Level"></select><select id="rank-mode" aria-label="Difficulty"></select></div><div id="rank-list" style="overflow-x:auto;margin:16px 0"></div><p>Score: 1,000 for completion + up to 1,000 for health + up to 1,000 for speed where a target time exists. Best run only; retries do not accumulate points.</p><button id="rank-back">Back</button></section>';
