...
export function workshop(save:Save){
 return `<div class="depot-heading"><h2>Tank systems</h2><span>20 LEVELS EACH</span></div><div class="upgrade-grid">${UPGRADES.map(id=>{
 ...
 }).join('')}</div><div class="depot-heading"><h2>Weapons</h2><span>INDIVIDUAL UPGRADES</span></div><p class="shop-weapon-help">Purchases equip next mission. Switch Gun or C / 1–8 in battle.</p><div class="upgrade-grid weapon-shop">${WEAPONS.map((w,id)=>{
  ...
  return `<article class="upgrade-card" data-weapon-card="${id}"><span class="eyebrow">${owned?'OWNED':'LOCKED'} · LEVEL ${level} / ${UPGRADE_CAP}</span><h3>${w.name}</h3><p>${w.description}</p><small class="weapon-stat">${damage}${owned&&!max?` → ${nextDamage}`:''} damage${id===5?` × ${barrels(save,id)}`:id===7?' × 3':''} · ${reloadSeconds(save,id).toFixed(2)}s reload</small><button data-action="${owned?'upgrade-weapon':'buy-weapon'}" data-value="${id}" ${owned&&max||save.credits<cost?'disabled':''}>${owned&&max?'MAX LEVEL':`${owned?'UPGRADE':'BUY'} · ${cost} CR`}</button></article>`;
 }).join('')}</div>`;
}
