...
export class CombatEffects {
  ...
  muzzle(p:T.Vector3,heading:number,rocket=false){
    ...
    for(let i=0;i<(this.low?1:4);i++)this.emit(p,'spark',0xffdb82,.09,.2,new T.Vector3(Math.sin(heading)*13+(Math.random()-.5)*4,Math.random()*2,Math.cos(heading)*13+(Math.random()-.5)*4));
  ...
  impact(p:T.Vector3,large=false){
    ...
    for(let i=0;i<(this.low?3:large?22:8);i++){
  ...
  update(dt:number,camera:T.Camera){
    while(this.particles.length>this.limit)this.recycle(this.particles.pop()!);
    this.trimPool(Math.max(0,this.limit-this.particles.length));
    ...
    for(let i=this.particles.length-1;i>=0;i--){const p=this.particles[i];p.age+=dt;
      if(p.age>=p.life){this.particles.splice(i,1);this.recycle(p);continue;}
