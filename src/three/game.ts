...
import {DROP_CHANCE,AMMO_CAPS,salvageReward} from './loot';
...
export class Game {
  ...
  special=new SpecialWeapons();specialAmmo=[0,0,0];infantryKills=0;
  weaponPickerOpen=false;supportPickerOpen=false;airSupport=new AirSupport();
  ...
  strikes:{x:number;z:number;time:number;marker:T.Mesh;bomb:T.Mesh}[]=[];barrageZones:T.Mesh[]=[];
  ...
  constructor(root:HTMLElement){
    ...
    root.innerHTML=`<div id="battlefield"></div><div class="vignette"></div><div id="hud" hidden>
      ...
      <button class="weapon-block" id="weapon"><span id="weapon-label"></span><strong id="reload-label"></strong><div class="reload-track"><i id="reload-fill"></i></div><small class="switch-gun-label">SWITCH GUN <span>· C / 1–8</span> ▴</small></button>
      <div class="abilities"><button id="fire" class="pc-fire"><kbd>SPACE / F</kbd>FIRE</button><button id="artillery" aria-controls="support-picker" aria-expanded="false"><kbd>R</kbd><span id="artillery-label">AIR SUPPORT</span></button><button id="shield"><kbd>Q</kbd><span id="shield-label">SHIELD</span></button><button id="repair"><kbd>E</kbd><span id="repair-label">FIND REPAIR</span></button></div></div>
      ...
      <div id="support-picker" class="weapon-picker support-picker" hidden role="group" aria-label="Choose air support"><button data-support="support-strike"><strong>1 · RING BARRAGE</strong><small>Around your tank · center clear</small></button><button data-support="support-drop"><strong>2 · SUPPLY DROP</strong><small id="supply-allowance"></small></button></div><div id="weapon-picker" class="weapon-picker" hidden role="group" aria-label="Choose weapon"></div><div class="radio" id="radio" role="status"></div><div class="desktop-hint">WASD <span>drive</span> · IJKL / MOUSE <span>aim</span> · SPACE / F <span>fire</span> · C / 1–8 <span>guns</span> · R <span>air support</span> · Q <span>shield</span> · E <span>find repair</span></div>
    ...
    this.input.onPause=()=>{if(this.phase==='playing'&&this.supportPickerOpen){this.supportPickerOpen=false;this.updateHud();}else if(this.phase==='playing')this.pause();else if(this.phase==='paused'&&!document.hidden&&document.hasFocus())this.resume();};
    this.input.onBackground=()=>{if(this.phase==='playing')this.pause();};
    this.input.onAction=a=>this.action(a);
    this.bindActions(this.hud,button=>{if(button.dataset.quickWeapon)this.supportPickerOpen=false;if(button.id==='pause')this.pause();else this.action(button.dataset.support??button.dataset.quickWeapon??button.dataset.weapon??(button.id==='weapon'?'switch':button.id));});
  ...
  prepare(index:number,level=unlockedLevel(this.save,index)){
    ...
    this.airSupport.clear();this.supportPickerOpen=false;this.hazards.reset(index);this.bosses.clear();this.special.clear();this.specialAmmo=[this.save.weapons.includes(3)?12:0,this.save.weapons.includes(4)?6:0,this.save.weapons.includes(7)?3:0];this.infantryKills=0;this.fieldWeaponCount=1;this.artilleryCooldown=0;this.powerBoost=0;this.clearBarrage();this.mission=index;this.world.build(index,m.kind,this.level,this.save.difficulty);this.encounters.reset();this.salvageDrops=0;this.shots=[];this.enemies=[];this.convoy=null;this.convoyBlocked=false;this.convoyDistance=0;this.convoyReverse=null;this.world.loopReverse=false;
    ...
    if(m.kind==='escort'){this.convoy=this.world.clone('transport');this.convoy.position.set(route.points[0].x,0,route.points[0].z);this.convoy.rotation.y=route.heading;this.world.entities.add(this.convoy);}
    ...
    const primary=bossKind(index),reinforcements:BossKind[]=index>=10?['quad-mech','siege-mech','missile-truck','helicopter','spider','laser']:['helicopter','spider','laser'];const kinds:BossKind[]=[primary,...(index>=10?reinforcements.filter(kind=>kind!==primary):reinforcements)];for(let i=0;i<bosses;i++)deploy(i,bosses,'boss',kinds[i]);
    ...
    this.enemies.forEach((u,i)=>assignPatrol(this,u,i));
  ...
  start(index:number,level=unlockedLevel(this.save,index)){this.prepare(index,level);this.setPhase('playing');this.world.target.copy(this.player.visual.root.position);const offset=this.world.cameraOffset(this.player.visual.root.position);this.world.target.x+=offset.x;this.world.target.z+=offset.z;this.world.update(0,this.player.visual.root.position);this.updateHud();this.overlay.innerHTML='';this.radioMessage(index===0?'IVO / Tap SWITCH GUN or press C. Tank wrecks may refill special weapons.':this.missionData().radio,9);this.last=performance.now();this.accumulator=0;this.tone(360,.12,.05);}
  action(action:string){
    ...
    if(this.supportPickerOpen&&['1','2'].includes(action))action=action==='1'?'support-strike':'support-drop';
    if(action==='artillery'){this.supportPickerOpen=!this.supportPickerOpen;this.weaponPickerOpen=false;}
    if(action==='support-strike'&&this.artilleryCooldown<=0){this.callArtillery();this.supportPickerOpen=false;}
    if(action==='support-drop'&&this.airSupport.request(this))this.supportPickerOpen=false;
    if(action==='shield'&&this.shieldCooldown<=0){this.shieldTime=Math.max(this.shieldTime,getSkin(this.save.skin).shield+shieldBonus(this.save));this.shieldCooldown=upgradedShieldCooldown(this.save);this.radioMessage(`KESTREL / Protective field active · ${this.shieldTime.toFixed(1)}s of cover.`,3);this.tone(620,.2,.04);}
    if(action==='repair'){
    ...
    if(action==='switch'){this.weaponPickerOpen=!this.weaponPickerOpen;this.supportPickerOpen=false;}
    if(/^[1-8]$/.test(action)){const w=Number(action)-1;if(this.weaponAvailable(w)){this.weapon=w;this.weaponPickerOpen=false;this.supportPickerOpen=false;if(ownsWeapon(this.save,w)){this.save.equippedWeapon=w;this.persist();}}else if(w>=3)this.radioMessage(w===7?'ARMORY / Triple arc: buy in the shop. Three volleys refill next mission.':ownsWeapon(this.save,w)?'ARMORY / Ammo empty. Collect a map cache or resupply next mission.':w>=5?'ARMORY / Buy this weapon in the shop.':'ARMORY / Buy in the shop or collect a map cache.',3);else this.radioMessage(`ARMORY / ${w===1?'Autocannon unlocks after First Light.':'Rockets unlock after Homeward.'}`,3);}
    this.updateHud();
  }
  ...
  private selectFallbackWeapon(){
    ...
    let next=this.weapon-1;while(next>0&&!this.weaponAvailable(next))next--;
  ...
  shoot(unit:Unit,friendly:boolean){
    ...
    if(friendly&&ammo!==undefined){
      ...
      this.specialAmmo[ammo]--;this.reload=this.reloadDuration();this.special.fire(this,this.weapon);this.shotsFired++;
  ...
  bossRound(unit:Unit,heading:number,damage:number,muzzleName='LightMuzzle',speed=38){
    ...
    this.shots.push({mesh,p,from:{...p},dx:Math.sin(shotHeading),dz:Math.cos(shotHeading),damage,speed,life:42/speed,friendly:false,splash:0,height:{start:mesh.position.y,range,traveled:0}});this.world.fx.muzzle(mesh.position,shotHeading,false);
  }
  updatePlayer(dt:number){
    ...
    const blast=WEAPONS[this.weapon].splash;this.world.cursor.scale.setScalar(blast?blast/.78:1);(this.world.cursor.material as T.MeshBasicMaterial).color.setHex(WEAPONS[this.weapon].arc?0xc392ff:blast?0xffbb78:0xc0ffdf);
  ...
  hitCover(cover:Cover,damage:number,laser=false){
...
if(cover.kind==='barrel'||cover.kind==='fuelcrate'){this.explode(cover,cover.kind==='fuelcrate'?7:5,cover.kind==='fuelcrate'?110:65);}}
  ...
  clearBarrage(){
    for(const s of this.strikes){s.bomb.removeFromParent();s.marker.removeFromParent();s.marker.geometry.dispose();(s.marker.material as T.Material).dispose();}
    for(const mesh of this.barrageZones){mesh.removeFromParent();mesh.geometry.dispose();(mesh.material as T.Material).dispose();}
    this.strikes=[];this.barrageZones=[];
  }
  callArtillery(){
    if(this.phase!=='playing'||this.player.dead||this.artilleryCooldown>0||this.strikes.length)return;
    const origin=this.player.visual.root.position.clone(),targets=barrageTargets(origin);
    if(!targets.length)return;
    this.artilleryCooldown=BARRAGE.cooldown;
    for(const [inner,outer,color,opacity] of [[BARRAGE_INNER,BARRAGE.ring+BARRAGE.blast,0xffb458,.055],[BARRAGE_INNER-.12,BARRAGE_INNER,0x9affd4,.85]]){
      const mesh=new T.Mesh(new T.RingGeometry(inner,outer,96),new T.MeshBasicMaterial({color,side:T.DoubleSide,transparent:true,opacity,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-1,polygonOffsetUnits:-1}));
      mesh.name=opacity<.1?'BarrageBand':'BarrageInnerBoundary';mesh.rotation.x=-Math.PI/2;mesh.position.set(origin.x,.13,origin.z);mesh.userData.owned=true;this.world.entities.add(mesh);this.barrageZones.push(mesh);
    }
    for(const {x,z,time} of targets){
      const marker=new T.Mesh(new T.RingGeometry(BARRAGE.blast-.16,BARRAGE.blast,48),new T.MeshBasicMaterial({color:0xffb458,side:T.DoubleSide,transparent:true,opacity:.8,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-1,polygonOffsetUnits:-1}));marker.rotation.x=-Math.PI/2;marker.position.set(x,.15,z);marker.userData.owned=true;this.world.entities.add(marker);
      const bomb=this.world.rocket();bomb.name='BarrageBomb';bomb.scale.setScalar(.85);bomb.rotation.x=Math.PI/2;bomb.position.set(x,26,z);bomb.visible=false;this.world.entities.add(bomb);
      this.strikes.push({x,z,time,marker,bomb});
    }
    this.radioMessage('AIR SUPPORT / Ring barrage inbound. Center clear; amber zones will be hit.',3);
  }
  updateActivities(dt:number){
    if(this.player.dead)return;
    this.artilleryCooldown=Math.max(0,this.artilleryCooldown-dt);this.powerBoost=Math.max(0,this.powerBoost-dt);this.airSupport.update(this,dt);
    const p=this.player.visual.root.position;
    this.dustClock-=dt;
    if(this.dustClock<=0){
    ...
    for(const activity of this.world.activities){
      ...
      if((activity.kind==='laser'||activity.kind==='arc')&&near<2.5&&this.specialAmmo[activity.kind==='laser'?0:1]<AMMO_CAPS[activity.kind==='laser'?0:1]){activity.spent=true;activity.mesh.visible=false;const w=activity.kind==='laser'?3:4;const added=Math.min(activity.amount,AMMO_CAPS[w-3]-this.specialAmmo[w-3]);this.specialAmmo[w-3]+=added;this.weapon=w;this.reload=0;this.world.burst(p,3);this.radioMessage(`ARMORY / +${added} ${w===3?'laser shots':'arc rockets'}. ${w===3?'Pierces 1 concrete.':'Flies over cover.'}`,4);}
    ...
    for(let i=this.strikes.length-1;i>=0;i--){
      ...
      if(strike.time<.5&&strike.time>0){const t=1-strike.time/.5;strike.bomb.visible=true;strike.bomb.position.y=.8+26*(1-t*t);}
      if(strike.time<=0){this.world.fx.impact(new T.Vector3(strike.x,.8,strike.z),true);this.explode(strike,BARRAGE.blast,BARRAGE.damage);this.tone(38,.3,.08);strike.bomb.removeFromParent();strike.marker.removeFromParent();strike.marker.geometry.dispose();(strike.marker.material as T.Material).dispose();this.strikes.splice(i,1);}
    }
    if(!this.strikes.length&&this.barrageZones.length)this.clearBarrage();
  }
  ...
  updateHud(){
    ...
    if(m.kind==='escort')objective=`TRANSPORT ${Math.max(0,Math.ceil(this.convoyHealth/2.6))}% · ${this.convoy&&distance(this.player.visual.root.position,this.convoy.position)<12?(this.world.layout.closed&&this.convoyReverse===null?'CHOOSE LEFT OR RIGHT':this.convoyBlocked?'CLEAR THE ROAD':'MOVING TO EXTRACTION'):'MOVE CLOSER TO ESCORT'}`;
    ...
    this.el('artillery-label').textContent=this.artilleryCooldown>0?`SUPPORT ${Math.ceil(this.artilleryCooldown)}s`:'AIR SUPPORT';
    this.el('artillery').setAttribute('aria-expanded',String(this.supportPickerOpen));this.el('support-picker').hidden=!this.supportPickerOpen;
    const dropLimit=this.airSupport.limit(this),remaining=this.airSupport.remaining(this),waiting=this.artilleryCooldown>0;
    this.el('supply-allowance').textContent=dropLimit?`${remaining} left · ${waiting?'radio recharging':'nearby landing'}`:'Mission allowance used';
    this.hud.querySelector<HTMLButtonElement>('[data-support="support-strike"]')!.disabled=waiting;
    this.hud.querySelector<HTMLButtonElement>('[data-support="support-drop"]')!.disabled=waiting||remaining===0;
  ...
  showShop(){
    ...
    this.overlay.innerHTML=`<section class="panel depot-panel"><span class="eyebrow">SUPPLY DEPOT / PERMANENT UPGRADES</span><div class="debrief-title"><h1>Field shop</h1><span class="credit-total">${this.save.credits}<small>SUPPLY CREDITS</small></span></div><nav class="shop-tabs"><button data-action="shop-tab" data-value="equipment" aria-pressed="true">UPGRADES & WEAPONS</button><button data-action="shop-tab" data-value="skins">TANK SKINS</button></nav>${workshop(this.save)}<button data-action="shop-back">← BACK</button><button class="primary" data-action="next">NEXT BRIEFING →</button><button class="quiet" data-action="menu">Return to command</button>${this.saveWarning?'<p>Storage is unavailable; keep this tab open to retain progress.</p>':''}</section>`;this.focusPrimary();
  }
  showSkins(){
    this.overlay.innerHTML=`<section class="panel depot-panel"><div class="debrief-title"><h1>Tank skins</h1><span class="credit-total">${this.save.credits}<small>SUPPLY CREDITS</small></span></div><nav class="shop-tabs"><button data-action="shop-tab" data-value="equipment">UPGRADES & WEAPONS</button><button data-action="shop-tab" data-value="skins" aria-pressed="true">TANK SKINS</button></nav><p>Buy once. Equip one skin. Bonuses apply next mission.</p><div class="skin-grid">${SKINS.map(skin=>{const owned=this.save.skins.includes(skin.id),selected=this.save.skin===skin.id;return `<article class="skin-card ${selected?'selected':''}"><img src="${import.meta.env.BASE_URL}skins/${skin.id}.png" alt="${skin.name} tank paint"/><h2>${skin.name}</h2>${'stars' in skin?`<span class="skin-stars" aria-label="${skin.stars} stars">${'★'.repeat(skin.stars)}</span>`:''}<p>${skin.bonus}</p><button data-action="${owned?'skin-equip':'skin-buy'}" data-value="${skin.id}" ${selected||!owned&&this.save.credits<skin.price?'disabled':''}>${selected?'EQUIPPED':owned?'EQUIP':`BUY & EQUIP · ${skin.price} CR`}</button></article>`;}).join('')}</div><button class="primary" data-action="shop-back">DONE →</button></section>`;this.focusPrimary();
  }
