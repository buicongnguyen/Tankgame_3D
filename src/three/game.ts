...
export class Game {
  ...
  flame=new Flamethrower();special=new SpecialWeapons();specialAmmo=[0,0,0];infantryKills=0;
  ...
  constructor(root:HTMLElement){
    ...
    root.innerHTML=`<div id="battlefield"></div><div class="vignette"></div><div id="hud" hidden>
      ...
      <div id="support-picker" class="weapon-picker support-picker" hidden role="group" aria-label="Choose air support"><button data-support="support-strike"><strong>1 · RING BARRAGE</strong><small>Around your tank · center clear</small></button><button data-support="support-drop"><strong>2 · SUPPLY DROP</strong><small id="supply-allowance"></small></button></div><div id="weapon-picker" class="weapon-picker" hidden role="group" aria-label="Choose weapon"></div><div class="radio" id="radio" role="status"></div><div class="desktop-hint">WASD <span>drive</span> · IJKL / MOUSE <span>aim</span> · SPACE / F <span>fire</span> · C / 1–9 <span>guns</span> · R <span>air support</span> · Q <span>shield</span> · E <span>find repair</span></div>
  ...
  prepare(index:number,level=unlockedLevel(this.save,index)){
    ...
    this.airSupport.clear();this.supportPickerOpen=false;this.hazards.reset(index);this.bosses.clear();this.flame.clear();this.special.clear();this.specialAmmo=[this.save.weapons.includes(3)?12:0,this.save.weapons.includes(4)?6:0,this.save.weapons.includes(7)?3:0];this.infantryKills=0;this.fieldWeaponCount=1;this.artilleryCooldown=0;this.powerBoost=0;this.clearBarrage();this.mission=index;this.world.build(index,m.kind,this.level,this.save.difficulty);this.encounters.reset();this.salvageDrops=0;this.shots=[];this.enemies=[];this.convoy=null;this.convoyBlocked=false;this.convoyDistance=0;this.convoyReverse=null;this.world.loopReverse=false;
  ...
  shoot(unit:Unit,friendly:boolean){
    ...
    if(friendly&&info.flame){if(!this.weaponAvailable(this.weapon))return;this.flame.fire(this);this.shotsFired++;this.reload=this.reloadDuration();return;}
