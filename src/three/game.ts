...
export class Game {
  ...
  prepare(index:number,level=unlockedLevel(this.save,index)){
    ...
    this.elapsed=0;this.capture=0;this.kills=0;this.shotsFired=0;this.reload=0;this.weapon=this.save.equippedWeapon;this.shieldTime=0;this.shieldCooldown=0;this.relayHealth=300;this.convoyHealth=260;
...
if(cover.kind==='barrel'||cover.kind==='fuelcrate'){this.explode(cover,cover.kind==='fuelcrate'?7:5,cover.kind==='fuelcrate'?110:65);}}
  ...
  objectiveProgress(){const m=this.missionData();if(m.kind!=='escort'&&this.battlefieldClear())return 1;let progress=0;
  ...
  updateHud(){
    ...
    if(m.kind==='escort')objective=`TRANSPORT ${Math.max(0,Math.ceil(this.convoyHealth/2.6))}% · ${this.convoy&&distance(this.player.visual.root.position,this.convoy.position)<12?(this.world.layout.closed&&this.convoyReverse===null?'CHOOSE LEFT OR RIGHT':this.convoyBlocked?'CLEAR THE MARKED PATH':'MOVING TO EXTRACTION'):'MOVE CLOSER TO ESCORT'}`;
    ...
    if(m.kind!=='escort'&&this.battlefieldClear())objective='AREA SECURED';
