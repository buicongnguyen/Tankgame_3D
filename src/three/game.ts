...
export class Game {
  ...
  constructor(root:HTMLElement){
    ...
    this.bindActions(this.hud,button=>{if(button.id==='pause')this.pause();else this.action(button.dataset.support??button.dataset.weapon??(button.id==='weapon'?'switch':button.id));});
  ...
  shoot(unit:Unit,friendly:boolean){
    ...
    if(friendly&&ammo!==undefined){
      ...
      if(this.specialAmmo[ammo]===0){this.weapon=0;this.radioMessage('ARMORY / Special ammo empty. Cannon selected.',3);}this.updateHud();return;
    }
...
if(cover.kind==='barrel'||cover.kind==='fuelcrate'){this.explode(cover,cover.kind==='fuelcrate'?7:5,cover.kind==='fuelcrate'?110:65);}}
  ...
  updateHud(){
    ...
    this.el('supply-allowance').textContent=dropLimit?`${remaining} left · ${waiting?'radio recharging':'nearby landing'}`:'Hard / Crazy relay missions';
