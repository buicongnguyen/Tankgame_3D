...
export class Game {
  ...
  constructor(root:HTMLElement){
    ...
    this.overlay.addEventListener('change',event=>{
      ...
      if(!(select instanceof HTMLSelectElement)||select.id!=='tank-flag'||this.phase!=='depot')return;
      this.save.flag=normalizeFlag(select.value);this.persist();this.world.applySkin(this.player.visual.root,this.save.skin,this.save.flag);
      this.el('flag-preview').innerHTML=flagPreview(this.save.flag);
    });
  ...
  async menuAction(action:string,value?:string){
    ...
    if(action==='skin-equip'&&this.phase==='depot'&&this.save.skins.includes(value??'')){this.save.skin=value!;this.persist();this.world.applySkin(this.player.visual.root,this.save.skin,this.save.flag);this.showShop();}
    ...
    if(action==='skirmish-setup'&&['menu','depot','failed'].includes(this.phase)){this.leaveTraining();this.prepare(this.save.mission);this.showSkirmishSetup();return;}
    if(action.startsWith('sk-')&&this.phase==='menu'&&this.overlay.querySelector('.skirmish-panel')){
      const draft=this.skirmishDraft,n=Number(value);
      if(action==='sk-map'&&Number.isInteger(n)&&n>=0&&n<MISSIONS.length){const maps=draft.maps.includes(n)?draft.maps.filter(m=>m!==n):[...draft.maps,n].sort((a,b)=>a-b);if(maps.length)draft.maps=maps;}
      if(action==='sk-speed'&&Number.isInteger(n)&&n>=0&&n<AI_SPEEDS.length)draft.speed=n;
      if(action==='sk-teams'&&Number.isInteger(n)&&n>=0&&n<TEAMS.length)draft.teams=n+1;
      if(action==='sk-size'&&Number.isInteger(n)&&n>=0&&n<TEAM_SIZES.length)draft.size=n;
      if(action==='sk-field'&&Number.isInteger(n)&&n>=0&&n<FIELDS.length)draft.field=n;
      storeSkirmish(draft);
      if(action==='sk-start'){this.startSkirmish();return;}
      const scroll=this.overlay.scrollTop;this.showSkirmishSetup();this.overlay.scrollTop=scroll;return;
    }
    if(action==='skirmish-next'&&this.skirmish&&!this.skirmish.last){this.skirmish.round++;this.startSkirmishBattle();return;}
    ...
    if(action==='retry'){if(this.training)this.startTraining(this.training.id);else if(this.skirmish)this.startSkirmishBattle();else this.start(this.mission,this.level);}
  ...
  showProfiles(editing:number|null=null){
    ...
    const saves=book.slots.map((slot,i)=>i===book.active?this.save:slot.save?parseSave(slot.save):null);
    this.setPhase('menu');this.overlay.innerHTML=profilesPanel({book,saves,names:book.slots.map((_,i)=>this.profileName(i)),editing});this.overlay.scrollTop=0;
    if(editing===null){this.focusPrimary();return;}
    const input=this.overlay.querySelector<HTMLInputElement>('#profile-name');
  ...
  startSkirmish(){this.leaveTraining();this.campaignSave=this.save;this.skirmish=new SkirmishSession(normalizeSkirmish(this.skirmishDraft));this.startSkirmishBattle();}
  /** Each battle borrows the player's tank through a throwaway copy of the save: upgrades, weapons and skin
   *  carry over, Strike gets a fixed two charges, and nothing is written back to the campaign. */
  startSkirmishBattle(){const s=this.skirmish!;this.save={...structuredClone(this.campaignSave!),strikeCharges:2,autoPack:0};this.start(s.map,1);}
  showSkirmishResult(){this.setPhase('depot');this.overlay.innerHTML=skirmishResult(this.skirmish!);this.focusPrimary();}
  ...
  makeUnit(x:number,z:number,role:Unit['role'],kind:BossKind=bossKind(this.mission)):Unit{
    ...
    const search=()=>{if(free(x,z))return;const origin={x,z};for(let r=3;r<=120;r+=3)for(let a=0;a<16;a++){const px=origin.x+Math.cos(a*Math.PI/8)*r,pz=origin.z+Math.sin(a*Math.PI/8)*r;if(free(px,pz)){x=px;z=pz;return;}}};
    search();let placed=free(x,z);
    // A cramped map can leave no room outside the deployment zone: fall back to the unguarded placement.
    if(!placed&&this.deploying){this.deploying=false;search();placed=free(x,z);this.deploying=true;}
    const visual=this.world.tank(!player,boss,infantry?role:role==='jeep'?'scout-jeep':role==='airlift'?'airlift':boss?'boss-'+kind:'tank');visual.root.position.set(x,0,z);if(boss){const core=visual.root.getObjectByName('Core');if(core)core.visible=false;}
    const difficulty=mode(this.save.difficulty);
    const hp=player?(240+this.save.upgrades.armor*65)*difficulty.health:(boss?BOSS[kind].health:enemyHealth(role,this.mission,this.level))*difficulty.enemyHealth*(this.skirmish?SKIRMISH_TOUGHNESS[this.save.difficulty]??1:1);
    if(!placed)throw new Error(`No free spawn for ${role}`);
  ...
  prepare(index:number,level=unlockedLevel(this.save,index)){
    ...
    this.elapsed=0;this.capture=0;this.kills=0;this.shotsFired=0;this.reload=0;this.weapon=this.save.equippedWeapon;this.shieldTime=0;this.shieldCooldown=0;this.relayHealth=300;this.convoyHealth=ESCORT.health;this.escortDeploying=false;this.deployShieldUntil=0;
    // Skirmish starts mid-map so teams can come from every side; spawn search keeps it clear of cover and mines.
    const route=this.world.layout,spawn=this.skirmish?{x:0,z:0}:route.spawn;this.player=this.makeUnit(spawn.x,spawn.z,'player');this.player.heading=this.player.aim=route.heading;this.world.applySkin(this.player.visual.root,this.save.skin,this.save.flag);
    ...
    // Skirmish battles replace the campaign encounters with their own teams (deployed below).
    const encounter=this.skirmish?{armor:0,riflemen:0,rocketeers:0,infantry:0,jeeps:0,bosses:0}:this.training?{armor:this.training.definition.tanks,riflemen:this.training.definition.rifles,rocketeers:0,infantry:this.training.definition.rifles,jeeps:0,bosses:0}:encounterSize(index,this.level,this.save.difficulty),count=encounter.armor,bosses=encounter.bosses;this.armoredGoal=count+bosses+encounter.jeeps;
    const escortMeters=m.kind==='escort'?escortEncounterMeters(route):null;this.escortDeploying=!!escortMeters;
    // No enemy starts within reach of the deployment point (escorts keep their own 36 m zone).
    const reach=clamp(route.length*.6,18,30),start=this.player.visual.root.position;
    this.deployZone=this.training||this.skirmish||escortMeters?null:{x:start.x,z:start.z,vehicle:reach,infantry:reach-4};this.deploying=!!this.deployZone;
  ...
  start(index:number,level=unlockedLevel(this.save,index)){this.prepare(index,level);if(!this.training&&!this.skirmish){grantBackgroundStrikes(this.save,index);this.auto.ammo=ammoCapacity(this.save.skin,this.save.autoPack);this.save.autoPack=0;}this.persist();this.setPhase('playing');this.world.target.copy(this.player.visual.root.position);const offset=this.world.cameraOffset(this.player.visual.root.position);this.world.target.x+=offset.x;this.world.target.z+=offset.z;this.world.update(0,this.player.visual.root.position);this.updateHud();this.overlay.innerHTML='';this.radioMessage(this.training?this.training.mission().radio:this.skirmish?this.missionData().radio:index===0?'IVO / Tap SWITCH GUN or press C. Tank wrecks may refill special weapons.':this.missionData().radio,9);this.last=performance.now();this.accumulator=0;this.tone(360,.12,.05);this.openingSupport();}
  /** Removes the opening grace (deployment shield, starter cache still in the air), for scripted scenarios that need raw combat from the first frame. */
  clearOpening(){this.shieldTime=0;this.deployShieldUntil=0;this.deployZone=null;for(const d of this.airSupport.drops)d.activity.mesh.removeFromParent();this.world.activities=this.world.activities.filter(a=>!a.airborne);this.airSupport.clear();}
  /** Opening grace outside training: a deployment shield on every mission, plus a small free cache on Easy. */
  openingSupport(){
    if(this.training)return;
    const guard=mode(this.save.difficulty).deployShield;this.shieldTime=guard;this.deployShieldUntil=this.elapsed+guard;
  ...
  unitRadius(unit:Unit){return this.isInfantry(unit)?.55:unit.role==='airlift'?2.8:unit.role==='boss'?BOSS[unit.bossKind??bossKind(this.mission)].radius:1.25;}
  ...
  updatePlayer(dt:number){
    ...
    if(WEAPONS[this.weapon].arc){const target=rocketTarget(p,this.aimPoint,this.enemies.filter(e=>!e.dead&&!e.pending).map(e=>e.visual.root.position),this.input.touchAiming||this.input.hasTouchAim,this.world.bounds);this.aimPoint.set(target.x,0,target.z);}
  ...
  updateEnemies(dt:number){
    ...
    for(let i=0;i<this.enemies.length;i++){
      ...
      // Skirmish teams move on their commander's orders; aiming and firing below stay shared.
      const ordered=!!this.skirmish&&this.skirmish.steer(this,e,dt,playerVisible);
      if(!target){e.cooldown=Math.max(e.cooldown,AIM_WARNING_SECONDS);if(e.patrol&&e.encounter){e.encounter.active=false;e.patrol.index=0;e.patrol.returning=true;e.patrol.wait=.6;e.patrol.waypoint=undefined;}e.visual.beam.visible=false;if(!ordered)this.moveUnit(e,0,0,dt);this.syncVisual(e);continue;}
  ...
  /** Base ground speed per role; skirmish battles scale it by the chosen AI speed. */
  enemySpeed(e:Unit){return (this.isInfantry(e)?2.4:e.role==='heavy'?2.9:e.role==='jeep'?4.8:4)*(this.skirmish?.pace??1);}
  ...
  airborne(unit:Unit){return (unit.role==='airlift'||unit.bossKind==='helicopter'||unit.bossKind==='quadcopter'||unit.bossKind==='jet')&&unit.visual.root.position.y>2;}
...
if(cover.kind==='barrel'||cover.kind==='fuelcrate'){this.explode(cover,cover.kind==='fuelcrate'?7:5,cover.kind==='fuelcrate'?110:65);}}
  ...
  updateActivities(dt:number){
    ...
    for(const activity of this.world.activities){
      ...
      if((activity.kind==='laser'||activity.kind==='arc')&&near<2.5&&this.specialAmmo[activity.kind==='laser'?0:1]<ammoCaps(this.save.skin)[activity.kind==='laser'?0:1]){activity.spent=true;activity.mesh.visible=false;const w=activity.kind==='laser'?3:4;const added=Math.min(activity.amount,ammoCaps(this.save.skin)[w-3]-this.specialAmmo[w-3]);this.specialAmmo[w-3]+=added;
        // Training must leave the loan ammo available until the player learns to select it.
        if(!(this.training?.id===1&&!this.training.switched))this.weapon=w;
        this.reload=0;this.world.burst(p,3);this.radioMessage(`ARMORY / +${added} ${w===3?'laser shots':'arc rockets'}. ${w===3?'Pierces 1 concrete.':'Flies over cover.'}`,4);}
  ...
  battlefieldClear(){return this.enemies.length>0&&this.enemies.every(e=>e.dead||this.airlifts.departing(e));}
  objectiveProgress(){if(this.training)return this.training.ready(this)?1:0;if(this.skirmish)return this.battlefieldClear()?1:0;if(this.missionData().kind==='rescue')return this.allies.progress(this);const m=this.missionData();if(this.battlefieldClear())return 1;let progress=0;
  ...
  step(dt:number){
    if(this.phase==='finishing'){this.finishDelay=Math.max(0,this.finishDelay-dt);if(this.finishDelay<1e-6){if(this.training){this.showTrainingResult();return;}if(this.skirmish){this.showSkirmishResult();return;}if(this.level===2&&(this.mission===5||this.mission===8||this.mission===MISSIONS.length-1)){this.setPhase('victory');this.showVictory();}else this.showDepot();}return;}
  ...
  updateHud(){
    ...
    const m=this.missionData();this.el('mission-number').textContent=this.skirmish?`SKIRMISH ${this.skirmish.round+1}/${this.skirmish.battles}${this.skirmish.scale>1?' · LARGE':''} · ${AI_SPEEDS[this.skirmish.config.speed].label.toUpperCase()} AI · ${MODES[normalizeDifficulty(this.save.difficulty)??'normal'].label.toUpperCase()}`:`STAGE ${String(this.mission+1).padStart(2,'0')} · LEVEL ${this.level+1}/3 · ${MODES[normalizeDifficulty(this.save.difficulty)??'normal'].label.toUpperCase()}`;this.el('mission-name').textContent=m.name;
    ...
    if(this.skirmish&&!this.battlefieldClear())objective=this.skirmish.objective(this).full;
    ...
    if(this.skirmish)short=this.skirmish.objective(this).short;
    ...
    const badge=this.el('mission-compact');badge.textContent=this.training?`T${this.training.id+1}`:this.skirmish?`S${this.skirmish.round+1}`:`${this.mission+1}.${this.level+1}`;badge.setAttribute('aria-label',this.el('mission-number').textContent!);
    ...
    // Metres to minimap pixels; Large skirmish maps shrink every feature to fit the same panel.
    const sx=166/(this.world.bounds.x*2),sz=122/(this.world.bounds.z*2),map=(point:Point)=>({x:point.x*sx+88,y:point.z*sz+66});
    if(this.world.environment.biome==='river'){const q=map({x:0,z:28});ctx.fillStyle='#398d9f';ctx.fillRect(map({x:-this.world.bounds.x,z:0}).x,q.y,this.world.bounds.x*2*sx,8*sz);for(const x of [-35,0,35]){const b=map({x,z:26});ctx.fillStyle='#baac82';ctx.fillRect(b.x-9*sx/2,b.y,9*sx,12*sz);}}
    for(const r of terrainRegions(this.world.environment.biome)){const q=map(r);ctx.fillStyle=r.kind==='ice'?'#72bad0':'#936d3d';ctx.beginPath();ctx.ellipse(q.x,q.y,r.rx*sx,r.rz*sz,0,0,Math.PI*2);ctx.fill();}
    ...
    for(const c of this.world.covers){if(c.hp<=0)continue;const q=map(c);ctx.fillStyle=['pine','white-pine','palm','jungle-tree'].includes(c.kind)?'#51845b':['house','cityblock'].includes(c.kind)?'#ac7858':c.kind==='fuelcrate'?'#ff8955':'#667770';const w=Math.max(3,c.w*sx),h=Math.max(3,c.d*sz);ctx.fillRect(q.x-w/2,q.y-h/2,w,h);}
    ...
    // Landing zones pulse in the team colour until the airlift is down; airlifts draw as a larger diamond.
    for(const f of this.airlifts.flights){if(!f.marker.visible)continue;const q=map(f.lz);ctx.strokeStyle=f.unit.squad!==undefined?TEAMS[f.unit.squad].css:'#ff7a5d';ctx.globalAlpha=.55+.35*Math.sin(this.elapsed*6);ctx.beginPath();ctx.arc(q.x,q.y,5,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1;}
    for(const u of [this.player,...this.enemies,...this.allies.tanks.map(a=>a.unit)]){if(u.dead||u.aboard)continue;const q=map(u.visual.root.position);if(u.role==='airlift'){ctx.fillStyle=u.squad!==undefined?TEAMS[u.squad].css:'#ff7a5d';ctx.beginPath();ctx.moveTo(q.x,q.y-4);ctx.lineTo(q.x+4,q.y);ctx.lineTo(q.x,q.y+4);ctx.lineTo(q.x-4,q.y);ctx.closePath();ctx.fill();ctx.strokeStyle='#142328';ctx.stroke();continue;}ctx.fillStyle=u===this.player||u.team==='ally'?'#a4ffe0':u.squad!==undefined?TEAMS[u.squad].css:u.encounter&&!u.encounter.active?'#87685e':'#ff7a5d';ctx.beginPath();ctx.arc(q.x,q.y,u===this.player?3:this.isInfantry(u)?1.2:2,0,Math.PI*2);ctx.fill();}
  ...
  complete(){
    ...
    // Skirmish battles record their own series stats; no campaign credits, progress or rankings.
  ...
  shopNavigation(){return '<nav class="shop-tabs" aria-label="Shop categories">'+([['equipment','Weapons'],['systems','Tank'],['support','Supplies'],['skins','Skins']] as const).map(([id,label])=>`<button data-action="shop-tab" data-value="${id}" aria-pressed="${this.shopTab===id}">${label}</button>`).join('')+'</nav>';}
  showShop(){
    ...
    this.setPhase('depot');
    if(this.shopTab==='skins'){this.showSkins();this.overlay.scrollTop=scroll;return;}
    this.overlay.innerHTML=`<section class="panel depot-panel"><span class="eyebrow">SUPPLY DEPOT</span><div class="debrief-title"><h1>Field shop</h1><span class="credit-total">${this.save.credits}<small>CREDITS</small></span></div>${this.shopNavigation()}<div class="shop-category" data-category="${this.shopTab}">${workshop(this.save)}</div><button class="primary" data-action="shop-back">DONE →</button>${this.saveWarning?'<p>Storage is unavailable; keep this tab open to retain progress.</p>':''}</section>`;
    this.overlay.scrollTop=scroll;
  }
  showSkins(){
    this.overlay.innerHTML=`<section class="panel depot-panel"><div class="debrief-title"><h1>Tank skins</h1><span class="credit-total">${this.save.credits}<small>CREDITS</small></span></div>${this.shopNavigation()}<div class="flag-picker"><span id="flag-preview">${flagPreview(this.save.flag)}</span><label for="tank-flag">Country flag<small>Optional · cosmetic</small></label><select id="tank-flag">${TANK_FLAGS.map(f=>`<option value="${f.id}" ${this.save.flag===f.id?'selected':''}>${f.name}</option>`).join('')}</select></div><p>Buy once. Equip one. Bonuses apply next mission.</p><div class="skin-grid">${SKINS.map(skin=>{const owned=this.save.skins.includes(skin.id),selected=this.save.skin===skin.id;return `<article class="skin-card ${selected?'selected':''}"><img src="${import.meta.env.BASE_URL}skins/${skin.id}.png?v=vivid-2" alt="${skin.name} tank paint"/><h2>${skin.name}</h2>${'stars' in skin?`<span style="color:${'starColor' in skin?skin.starColor:'#ffe588'}" class="skin-stars" aria-label="${skin.stars} stars">${'★'.repeat(skin.stars)}</span>`:''}<p>${skin.bonus}</p><button data-action="${owned?'skin-equip':'skin-buy'}" data-value="${skin.id}" aria-label="${selected?'Equipped':owned?'Equip':'Buy and equip'} ${skin.name}${owned?'':` for ${skin.price} credits`}" ${selected||!owned&&this.save.credits<skin.price?'disabled':''}>${selected?'EQUIPPED':owned?'EQUIP':coin(skin.price)}</button></article>`;}).join('')}</div><button class="primary" data-action="shop-back">DONE →</button></section>`;
  }
  ...
  fail(){this.hazards.clear();this.clearLesson();this.setPhase('failed');if(this.skirmish){this.overlay.innerHTML=skirmishFailed(this.skirmish);this.focusPrimary();return;}this.overlay.innerHTML=`<section class="panel pause-panel"><span class="eyebrow">OPERATION INTERRUPTED</span><h1>We go again.</h1><p>${this.missionData().kind==='rescue'&&this.allies.saved===2&&this.allies.survivors===0?'Both rescued tanks were lost. Clear threats before restoring the crews.':this.convoyHealth<=0?'The rescue transport was lost. Stay close and intercept the ambushers.':this.relayHealth<=0?'The uplink was destroyed. Intercept enemies before they reach the relay.':'Kestrel is disabled. Use hard cover, keep your front toward incoming fire, and activate your shield before crossing a firing lane.'}</p><button class="primary" data-action="retry">RETRY ${this.missionData().name.toUpperCase()} →</button><button data-action="menu">Return to command</button><small>Your purchased upgrades and unlocked operations are safe.</small></section>`;this.focusPrimary();}
