...
export class Game {
  ...
  constructor(root:HTMLElement){
    ...
    root.innerHTML=`<div id="battlefield"></div><div class="vignette"></div><div id="hud" hidden>
      ...
      <div id="support-picker" class="weapon-picker support-picker" hidden role="group" aria-label="Choose air support"><button data-support="support-strike"><strong>1 · BARRAGE</strong><small>Five rounds · aimed area</small></button><button data-support="support-drop"><strong>2 · SUPPLY DROP</strong><small id="supply-allowance"></small></button></div><div id="weapon-picker" class="weapon-picker" hidden role="group" aria-label="Choose weapon"></div><div class="radio" id="radio" role="status"></div><div class="desktop-hint">WASD <span>drive</span> · IJKL / MOUSE <span>aim</span> · SPACE / F <span>fire</span> · C / 1–5 <span>guns</span> · R <span>air support</span> · Q <span>shield</span> · E <span>find repair</span></div>
  ...
  action(action:string){
    ...
    if(action==='shield'&&this.shieldCooldown<=0){this.shieldTime=Math.max(this.shieldTime,getSkin(this.save.skin).shield);this.shieldCooldown=14;this.radioMessage(`KESTREL / Protective field active · ${this.shieldTime}s of cover.`,3);this.tone(620,.2,.04);}
    if(action==='repair'){
  ...
  shoot(unit:Unit,friendly:boolean){
    ...
    const stats=friendly?this.weaponStats():{damage:unit.role==='rifleman'||unit.role==='jeep'?3:unit.role==='rocketeer'?24:unit.role==='boss'?33:unit.role==='heavy'?24:15,speed:unit.role==='rifleman'||unit.role==='jeep'?44:unit.role==='boss'?32:25,splash:unit.role==='rocketeer'?2.5:0};
    ...
    if(unit.role==='rifleman'||unit.role==='jeep')mesh.scale.set(.35,.35,.5);if(rocket)mesh.scale.setScalar(friendly?1.2:.85);
    ...
    if(unit.role==='rifleman'||unit.role==='jeep')this.world.fx.emit(mesh.position.clone(),'flash',0xffd494,.3,.07);else this.world.fx.muzzle(mesh.position,unit.aim,friendly&&this.weapon===2);
  }
...
if(cover.kind==='barrel'||cover.kind==='fuelcrate'){this.explode(cover,cover.kind==='fuelcrate'?7:5,cover.kind==='fuelcrate'?110:65);}}
  ...
  updateHud(){
    ...
    if(picker.dataset.state!==state){picker.dataset.state=state;picker.innerHTML=['CANNON','AUTOCANNON','ROCKETS','LASER','ARC ROCKET'].map((name,i)=>`<button data-weapon="${i+1}" aria-pressed="${this.weapon===i}" ${!this.weaponAvailable(i)?'disabled':''}><strong>${i+1} · ${name}</strong><small>${i>=3?(this.specialAmmo[i-3]>0?this.specialAmmo[i-3]+' shots · '+(i===3?'Pierces 1 concrete':'Over cover'):ownsWeapon(this.save,i)?'Empty · find ammo':'Shop / map cache'):i>=available?(i===1?'Clear First Light':'Clear Homeward'):['Heavy single shot','Rapid fire','Area damage'][i]}</small></button>`).join('');}
