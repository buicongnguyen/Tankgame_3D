...
export class Game {
  ...
  constructor(root:HTMLElement){
    ...
    root.innerHTML=`<div id="battlefield"></div><div class="vignette"></div><div id="hud" hidden>
      ...
      <div class="bottom-hud"><div class="hull-block"><div><span>KESTREL / HULL</span><strong id="health-label"></strong></div><div class="hull-track"><i id="health-fill"></i></div><small id="status-line">ARMOR ONLINE</small></div>
      <button class="weapon-block" id="weapon"><span id="weapon-label"></span><strong id="reload-label"></strong><div class="reload-track"><i id="reload-fill"></i></div><small>1 / 2 / 3 · SWITCH WEAPON</small></button>
  ...
  showMenu(){
    ...
    this.overlay.innerHTML=`<main class="command-screen"><header class="brand"><span class="brand-mark">◈</span><span>KESTREL DIVISION<small>MERIDIAN RECOVERY COMMAND</small></span><span class="build-label">3D CAMPAIGN / 01</span></header><section class="hero"><span class="eyebrow">SIX OPERATIONS. ONE WAY HOME.</span><h1>STEEL<br><em>FRONT</em><span>LAST SIGNAL</span></h1><p class="tagline">The valley went silent.<br>You are the answer.</p><div class="hero-status"><i></i> CREW READY <span>●</span> SIGNAL FOUND</div></section><aside class="briefing panel"><div class="panel-top"><span class="eyebrow">OPERATION ${String(this.mission+1).padStart(2,'0')} / 06</span><span class="sector">${m.sector}</span></div><h2>${m.name}</h2><p class="briefing-copy">${m.briefing}</p><div class="mission-task"><span>PRIMARY OBJECTIVE</span><strong>${m.objective}</strong></div><div class="difficulty" aria-label="Difficulty">${(['story','standard','veteran'] as Difficulty[]).map(d=>`<button data-action="difficulty" data-value="${d}" aria-pressed="${this.save.difficulty===d}" class="${this.save.difficulty===d?'active':''}">${d}</button>`).join('')}</div><button class="primary deploy" data-action="deploy">DEPLOY KESTREL <span>→</span></button><details><summary>Controls & field manual</summary>${this.controls()}<p class="manual">Front armor absorbs damage. Flank for stronger hits. Red lines warn of incoming fire. Amber marks the objective. Destroy red fuel drums to damage nearby units.</p></details></aside><section class="campaign-route"><div class="route-heading"><span class="eyebrow">THE ROAD HOME</span><span>${this.save.cleared.filter(Boolean).length} / 6 COMPLETE</span></div><div class="route-list">${this.route()}</div></section><footer>${this.settings()}<button class="quiet" data-action="reset-prompt">Reset campaign</button></footer>${this.saveWarning?'<p class="storage-warning">Browser storage is unavailable. Progress will last only for this session.</p>':''}</main>`;
    this.focusPrimary();
  }
  pause(){if(this.phase!=='playing')return;this.setPhase('paused');this.renderPause();}
  renderPause(){this.overlay.innerHTML=`<section class="panel pause-panel"><span class="eyebrow">UPLINK ON HOLD</span><h1>Take a breath.</h1><p>The battlefield is paused. Your crew is waiting.</p><button class="primary" data-action="resume">RESUME OPERATION →</button><button data-action="retry">Restart this operation</button><button data-action="menu">Return to command</button>${this.controls()}${this.settings()}</section>`;this.focusPrimary();}
