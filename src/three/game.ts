...
import {SkirmishSession,loadSkirmish,storeSkirmish,skirmishSetup,skirmishResult,skirmishFailed,TEAMS,AI_SPEEDS,TEAM_SIZES} from './skirmish';
...
export class Game {
  ...
  skirmish:SkirmishSession|null=null;skirmishDraft=loadSkirmish();
  ...
  showSkirmishSetup(){this.setPhase('menu');this.overlay.innerHTML=skirmishSetup(this.skirmishDraft,MODES[this.save.difficulty].label);this.focusPrimary();}
  startSkirmish(){this.leaveTraining();this.campaignSave=this.save;this.skirmish=new SkirmishSession({...this.skirmishDraft,maps:[...this.skirmishDraft.maps]});this.startSkirmishBattle();}
...
if(cover.kind==='barrel'||cover.kind==='fuelcrate'){this.explode(cover,cover.kind==='fuelcrate'?7:5,cover.kind==='fuelcrate'?110:65);}}
  ...
  updateHud(){
    ...
    const m=this.missionData();this.el('mission-number').textContent=this.skirmish?`SKIRMISH ${this.skirmish.round+1}/${this.skirmish.battles} · ${AI_SPEEDS[this.skirmish.config.speed].label.toUpperCase()} AI · ${MODES[normalizeDifficulty(this.save.difficulty)??'normal'].label.toUpperCase()}`:`STAGE ${String(this.mission+1).padStart(2,'0')} · LEVEL ${this.level+1}/3 · ${MODES[normalizeDifficulty(this.save.difficulty)??'normal'].label.toUpperCase()}`;this.el('mission-name').textContent=m.name;
