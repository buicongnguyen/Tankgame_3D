...
export function assignPatrol(g:Game,u:Unit,index:number){
 ...
 for(const p of candidates){
  if(g.deployZone&&distance(p,g.deployZone)<(infantry?g.deployZone.infantry:g.deployZone.vehicle))continue;
...
export function updatePatrol(g:Game,u:Unit,dt:number){
 ...
 if(g.deployZone&&!patrol.returning&&distance({x:p.x+dx,z:p.z+dz},g.deployZone)<(g.isInfantry(u)?g.deployZone.infantry:g.deployZone.vehicle)&&distance({x:p.x+dx,z:p.z+dz},g.deployZone)<distance(p,g.deployZone)){next();return;}
