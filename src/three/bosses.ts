...
export class BossCombat{
 ...
 update(g:Game,u:Unit,dt:number){
  ...
  if(s.phase==='tracking'){
   ...
   if(s.time<=0){if(kind==='spider')s.landing=undefined;s.phase='charging';s.time=cfg.charge;s.heading=aim;u.aim=aim;s.targets=kind==='missile'||kind==='helicopter'?[-1,0,1].map(side=>({x:clamp(target.x+side*BOSS[kind].targetSpread,-BOUNDS.x,BOUNDS.x),z:clamp(target.z+(side===0?3:-2),-BOUNDS.z,BOUNDS.z)})):[];
    for(const t of s.targets){const radius=(kind==='missile'||kind==='helicopter')?BOSS[kind].blastRadius:0;const marker=new T.Mesh(new T.RingGeometry(radius-.2,radius,48),new T.MeshBasicMaterial({color:0xff6655,transparent:true,opacity:.8,side:T.DoubleSide}));marker.rotation.x=-Math.PI/2;marker.position.set(t.x,.18,t.z);marker.renderOrder=2;(marker.material as T.Material).depthWrite=false;g.world.entities.add(marker);s.markers.push(marker);const rocket=g.world.rocket();rocket.visible=false;rocket.rotation.x=Math.PI/2;rocket.position.set(t.x,28,t.z);g.world.entities.add(rocket);s.rockets.push(rocket);}
   }
  }else if(s.phase==='charging'){
   s.time-=dt;u.aim=s.heading;
   if(kind!=='missile'&&kind!=='helicopter'){const beam=u.visual.beam;beam.visible=true;beam.scale.set(kind==='rail'?BOSS.rail.beamRadius*2/.08:kind==='walker'||kind==='spider'?3:1,1,52);beam.position.set(p.x+Math.sin(s.heading)*26,.13,p.z+Math.cos(s.heading)*26);beam.rotation.y=s.heading;}
   ...
   if(s.time<=0){
    ...
    if(kind==='missile'||kind==='helicopter')for(const t of s.targets){g.world.fx.impact(new T.Vector3(t.x,.8,t.z),true);g.explode(t,BOSS[kind].blastRadius,70);}
    if(kind==='walker'||kind==='spider'){for(const angle of [-.22,0,.22]){u.aim=s.heading+angle;g.shoot(u,false);}u.aim=s.heading;}
    for(const m of s.markers){m.removeFromParent();m.geometry.dispose();(m.material as T.Material).dispose();}s.markers=[];for(const r of s.rockets)r.removeFromParent();s.rockets=[];u.visual.beam.visible=false;s.phase=kind==='helicopter'?'landing':kind==='laser'?'firing':'exposed';s.time=kind==='laser'?1:cfg.recovery;s.burst=0;if(kind==='helicopter')s.landing=this.landingPoint(g,u);
   }
   ...
   if(s.burst<=0){s.burst+=BOSS.laser.pulseInterval;this.rail(g,u,s.heading,16,0x8bffff,BOSS.laser.beamRadius);}
