...
interface State{phase:'tracking'|'charging'|'exposed';time:number;heading:number;targets:Point[];markers:T.Mesh[];}
export class BossCombat{
 states=new Map<Unit,State>();
 cancel(unit:Unit){const state=this.states.get(unit);if(state)for(const m of state.markers){m.removeFromParent();m.geometry.dispose();(m.material as T.Material).dispose();}unit.visual.beam.visible=false;this.states.delete(unit);}
 ...
 update(g:Game,u:Unit,dt:number){
  ...
  let s=this.states.get(u);if(!s){s={phase:'tracking',time:2.2,heading:u.aim,targets:[],markers:[]};this.states.set(u,s);}
  ...
  if(s.phase==='tracking'){
   ...
   if(s.time<=0){s.phase='charging';s.time=cfg.charge;s.heading=aim;u.aim=aim;s.targets=kind==='missile'?[-5,0,5].map(dx=>({x:clamp(target.x+dx,-BOUNDS.x,BOUNDS.x),z:clamp(target.z+(dx===0?3:-2),-BOUNDS.z,BOUNDS.z)})):[];
    for(const t of s.targets){const marker=new T.Mesh(new T.RingGeometry(4.3,4.5,48),new T.MeshBasicMaterial({color:0xff6655,transparent:true,opacity:.8,side:T.DoubleSide}));marker.rotation.x=-Math.PI/2;marker.position.set(t.x,.1,t.z);g.world.entities.add(marker);s.markers.push(marker);}
   }
   ...
   if(kind==='missile'&&s.time<.4&&g.trailClock<=0)for(const t of s.targets)g.world.fx.emit(new T.Vector3(t.x,Math.max(1,s.time*45),t.z),'flash',0xff8b55,1.4,.15,new T.Vector3(0,-30,0));
   ...
   if(s.time<=0){
    ...
    for(const m of s.markers){m.removeFromParent();m.geometry.dispose();(m.material as T.Material).dispose();}s.markers=[];u.visual.beam.visible=false;s.phase='exposed';s.time=2.4;
   }
