...
export class BossCombat{
 ...
 update(g:Game,u:Unit,dt:number){
  ...
  if(s.phase==='tracking'){
   ...
   if(isMissile(kind)){
    const modern=kind==='siege-mech'||kind==='missile-truck'||kind==='quadcopter',flight=modern?1:.65;
   ...
   if(s.time<=0){
    ...
    if(isMissile(kind))for(const t of s.targets){g.world.fx.impact(new T.Vector3(t.x,.8,t.z),true);g.explode(t,BOSS[kind].blastRadius,kind==='missile-truck'?80:kind==='siege-mech'?60:kind==='quadcopter'?60:70);}
    if(kind==='walker'||kind==='spider'){for(const angle of [-.22,0,.22]){u.aim=s.heading+angle;g.shoot(u,false);}u.aim=s.heading;}
    for(const m of s.markers){m.removeFromParent();m.geometry.dispose();(m.material as T.Material).dispose();}s.markers=[];for(const r of s.rockets)r.removeFromParent();s.rockets=[];u.visual.beam.visible=false;s.phase=flyingBoss(kind)?'landing':kind==='laser'||kind==='quad-mech'?'firing':'exposed';s.time=kind==='laser'?1:kind==='quad-mech'?1.05:cfg.recovery;s.burst=0;s.rounds=0;if(flyingBoss(kind))s.landing=this.landingPoint(g,u);
   }
  ...
  }else{ s.time-=dt;if(s.time<=0){s.phase='tracking';s.time=cfg.tracking*(!flyingBoss(kind)&&u.hp<u.max*.5?.55:1);s.landing=undefined;} }
 ...
 climb(g:Game,u:Unit,dx:number,dz:number,dt:number){
  ...
  for(const c of g.world.covers)if(c.hp>0&&(circleBox(p,r,c)||circleBox(ahead,r,c)))height=Math.max(height,c.kind==='cityblock'?9:c.kind==='house'?5:c.kind==='hill'||c.kind==='glacier'?5:c.kind==='concrete-block'?3.7:c.kind==='volcano'?13:2.5);
