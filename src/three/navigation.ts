...
export class GroundNavigation {
 ...
 private coverSource?:Cover[];private liveCovers:Cover[]=[];
 ...
 next(world:World,from:Point,target:Point):Point|null{
  if(this.revision!==world.navigationRevision||this.coverSource!==world.covers){
   this.coverSource=world.covers;this.liveCovers=world.covers.filter(c=>c.hp>0);
   const covers=this.liveCovers;
   this.revision=world.navigationRevision;this.fields.clear();this.edges.fill(0);
   for(let id=0;id<this.free.length;id++)this.free[id]=Number(Math.abs(this.point(id).x)<world.bounds.x-this.radius&&Math.abs(this.point(id).z)<world.bounds.z-this.radius&&!covers.some(c=>circleBox(this.point(id),this.radius+.15,c)));
